...
MAZ_OFFSET = 0
CROP_SKIMS = True
CROP_SURVEY_DATA = False

survey_data_path = r'C:\ABM3_dev\estimation\survey_data'

segments = {
    "test": list(pd.read_csv('test_data_zones.csv').MAZ.values),
    # MAZs in zip code 92103 & 92101 (downtown San Diego)
    # "series15": \
    #     list(pd.read_csv(os.path.join(survey_data_path, 'downtown_zones.csv')).MGRA.values) + 
...
if CROP_SURVEY_DATA:
    ...
    trips  = pd.read_csv(os.path.join(survey_data_path, "override_trips.csv"))
    tours  = pd.read_csv(os.path.join(survey_data_path, "override_tours.csv"))
    persons  = pd.read_csv(os.path.join(survey_data_path, "override_persons.csv"))
    households  = pd.read_csv(os.path.join(survey_data_path, "override_households.csv"))
    joint_tour_participants  = pd.read_csv(os.path.join(survey_data_path, "override_joint_tour_participants.csv"))
